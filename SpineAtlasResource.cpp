...
//        print_line(vformat("Texture loaded."));
    }
...
//    print_line(vformat("Importing atlas file: %s", p_path));
    ...
    tex_list.clear();
    ntex_list.clear();
//    print_line(vformat("Try to load atlas:\n%s\nlen: %d\n", atlas_data, atlas_data.size()));
//
//    int len = 0;
//    auto temp_text = spine::DefaultSpineExtension::getInstance()->_readFile(spine::String(p_path.utf8()), &len);
//    print_line(vformat("Test load file using ext, text:\n%s\nlen: %d", temp_text, len));

    //atlas = new spine::Atlas(spine::String(p_path.utf8()), new RaiixSpineTextureLoader(&tex_list, &ntex_list, normal_texture_prefix));
    auto len = atlas_data.size()-1 >= 0 ? atlas_data.size()-1 : 0;
    atlas = new spine::Atlas(atlas_data.utf8(), len, source_path.get_base_dir().utf8(), new RaiixSpineTextureLoader(&tex_list, &ntex_list, normal_texture_prefix));
